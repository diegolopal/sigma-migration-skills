...
OptionParser.new do |p|
  ...
  p.on('--github-repo S', 'GitHub repo for issue filing (default: twells89/sigma-migration-skills)') { |v| opts[:github_repo] = v }
end.parse!
...
if result['status'] == 'ok'
  ...
  if %w[auto gh].include?(target) && system('which gh > /dev/null 2>&1')
    repo = opts[:github_repo] || 'twells89/sigma-migration-skills'
