...
<ul>
  ...
  <li><strong>The data-model converter backend</strong> — the Tableau→Sigma converter (<code>convert_tableau_to_sigma</code>). Two ways to provide it; pick based on your data-residency requirements:
    <ul>
      <li><strong>LOCAL (default — no data leaves your machine).</strong> Run the converter locally and set <code>TABLEAU_MCP_BUILD</code> to its <code>build/tableau.js</code>, or run the <code>sigma-data-model</code> MCP server locally and have your agent call it. <strong>Required for customers who cannot send <code>.twb</code> contents (schema, SQL, calc formulas) off-box.</strong> The orchestrator uses this whenever <code>TABLEAU_MCP_BUILD</code> points at a real file.</li>
      <li><strong>HOSTED (convenience — uploads the <code>.twb</code>).</strong> Add the hosted MCP to your agent at <code>https://sigma-data-model-mcp.onrender.com/mcp</code> (streamable HTTP), e.g. <code>claude mcp add --transport http sigma-data-model https://sigma-data-model-mcp.onrender.com/mcp</code>. The orchestrator only uses this when you explicitly consent with <code>--converter hosted</code> (or <code>SIGMA_CONVERTER_ALLOW_HOSTED=1</code>), because it sends the workbook's XML to that third-party server. Do <strong>not</strong> use it for customer data that must stay in-tenant.</li>
    </ul>
  </li>
